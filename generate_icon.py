#!/usr/bin/env python3
"""Generate Piston Extension's mod menu icon: an extended piston, composed
from the vanilla piston_side texture.
...
OUT = os.path.join(HERE, "src/main/resources/assets/piston-extension/icon.png")
...
def build_icon():
    """An extended piston seen from the side, cut from piston_side: the wooden
    head band on top, the arm below it, then the block body. The mod's own
    piston_shaft model textures itself from this same vanilla texture."""
    piston = vanilla("block/piston_side.png")
    canvas = blank(16, 16)

    # head plate: the wooden band that piston_side puts in its top four rows
    for y in range(4):
        for x in range(16):
            canvas[y][x] = piston[y][x]

    # arm: four wide, sampling the band so it keeps the vanilla wood grain
    for y in range(4, 8):
        for x in range(6, 10):
            canvas[y][x] = piston[(y - 4) % 4][2 + (x - 6)]

    # body: the grey piston block the arm is pushing out of
    for y in range(8, 16):
        for x in range(16):
            canvas[y][x] = piston[y][x]
